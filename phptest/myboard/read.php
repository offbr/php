	...
	<table width=100%>
		<tr>
			<td width=200 align=left height=20>
				<a href=list.php?no=<?="$_GET[no]"?>><font color=white>
				[목록보기]</font></a>
				<a href=write.php><font color=white>
				[글쓰기]</font></a>
				<a href=edit.php?id=<?="$_GET[id]"?>><font color=white>
				[수정]</font></a>
...
<?
	...
	// 즉 바로 이전 글
	$query=mysql_query("SELECT id FROM board WHERE id > '$_GET[id]' LIMIT 1",//
	$conn);
	...
	$query=mysql_query("SELECT id FROM board WHERE id < '$_GET[id]' 
...
<?
	// 조회수 업데이트
	$result=mysql_query("UPDATE board SET view=view+1 WHERE id='$_GET[id]'",
	$conn);
